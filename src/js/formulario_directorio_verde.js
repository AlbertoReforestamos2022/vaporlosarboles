    document.querySelector(".directorioForm").addEventListener("submit", async function(e) {
        e.preventDefault();
        ...
        try {
            ...
            if (data.success) {
                Msj_status('#mensaje-directorio-verde', 'bg-primary', data.data.message);
                const btnDescargar = document.querySelector('.btn-formulario-descargar');
                btnDescargar.classList.remove('d-none'); 

                const classForm = document.querySelector('.directorioForm'); 
                classForm.classList.add('d-none');  
            } else {
                const msgError = data.data?.message || 'Hubo un error al enviar el formulario.';
                Msj_status('#mensaje-directorio-verde', 'bg-danger', msgError);
                console.error('Error real de PHP/PHPMailer:', msgError);
            }
        } catch (error) {
            console.error('Error en el envío del formulario:', error);
            Msj_status('#mensaje-directorio-verde', 'bg-danger', 'Hubo un error al enviar el formulario. Revisa la consola.' );
        }
    });
